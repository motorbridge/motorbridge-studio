...
function preserveParamStreamFields(hit, next) {
  ...
  const preservedKeys =
    String(hit.vendor) === 'robstride'
      ? ['pos', 'vel', 'torq', 'iqf', 'vbus', 'run_mode', 'run_mode_name', 'feedback_source']
...
export function useMotorStudio() {
  ...
  const connection = useMemo(
    () => ({
      ...
      wsTokenEnabled: connectionState.wsTokenEnabled,
      setWsTokenEnabled: connectionState.setWsTokenEnabled,
      wsToken: connectionState.wsToken,
      setWsToken: connectionState.setWsToken,
  ...
  const robotArm = useMemo(
    () => ({
      ...
      exportArmParams: robotArmState.exportArmParams,
      importArmParams: robotArmState.importArmParams,
    }),
    [robotArmState]
  );
